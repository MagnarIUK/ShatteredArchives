...
public class ArcticiteDripstoneFeature
        extends Feature<LargeDripstoneFeatureConfig> {
    public ArcticiteDripstoneFeature(Codec<LargeDripstoneFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<LargeDripstoneFeatureConfig> context) {
        ...
        ArcticiteDripstoneFeature.DripstoneGenerator dripstoneGenerator = ArcticiteDripstoneFeature.createGenerator(blockPos.withY(bounded.getCeiling() - 1), false, random, k, largeDripstoneFeatureConfig.stalactiteBluntness, largeDripstoneFeatureConfig.heightScale);
        ArcticiteDripstoneFeature.DripstoneGenerator dripstoneGenerator2 = ArcticiteDripstoneFeature.createGenerator(blockPos.withY(bounded.getFloor() + 1), true, random, k, largeDripstoneFeatureConfig.stalagmiteBluntness, largeDripstoneFeatureConfig.heightScale);
        ArcticiteDripstoneFeature.WindModifier windModifier = dripstoneGenerator.generateWind(largeDripstoneFeatureConfig) && dripstoneGenerator2.generateWind(largeDripstoneFeatureConfig) ? new ArcticiteDripstoneFeature.WindModifier(blockPos.getY(), random, largeDripstoneFeatureConfig.windSpeed) : ArcticiteDripstoneFeature.WindModifier.create();
    ...
    static ArcticiteDripstoneFeature.DripstoneGenerator createGenerator(BlockPos pos, boolean isStalagmite, Random random, int scale, FloatProvider bluntness, FloatProvider heightScale) {
        return new ArcticiteDripstoneFeature.DripstoneGenerator(pos, isStalagmite, scale, bluntness.get(random), heightScale.get(random));
    }

    private void testGeneration(StructureWorldAccess world, BlockPos pos, CaveSurface.Bounded surface, ArcticiteDripstoneFeature.WindModifier wind) {
        ...
        while (mutable.getY() < surface.getCeiling() - 1) {
            ...
            if (canGenerate(world, blockPos) || world.getBlockState(blockPos).isOf(ShatteredBlocks.ARCTICITE_GLASS)) {
    ...
    static final class DripstoneGenerator {
        ...
        boolean canGenerate(StructureWorldAccess world, ArcticiteDripstoneFeature.WindModifier wind) {
            while (this.scale > 1) {
        ...
        private static double scaleHeightFromRadius(double radius, double scale, double heightScale, double bluntness) {
            if (radius < bluntness) {
                radius = bluntness;
            }
            double d = 0.384;
        ...
        void generate(StructureWorldAccess world, Random random, ArcticiteDripstoneFeature.WindModifier wind) {
                ...
                block1: for (int j = -this.scale; j <= this.scale; ++j) {
                    ...
                    for (int m = 0; m < k && mutable.getY() < l; ++m) {
                        ...
                        if (canGenerateOrLava(world, blockPos)) {
                            bl = true;
                            Block block = ShatteredBlocks.ARCTICITE_GLASS;
    ...
    static final class WindModifier {
        ...
        static ArcticiteDripstoneFeature.WindModifier create() {
            return new ArcticiteDripstoneFeature.WindModifier();
        }
